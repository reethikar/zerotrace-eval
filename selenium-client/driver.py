...
class StartDriver:
    ...
    def run_experiment(self, uniqname="skorman", network="UMich WiFi", vpn=False, vpn_loc="Michigan", your_loc="Ann Arbor, MI, USA"):
    ...
    def wait_for_completion(self):
        counter = 0
        while self.driver.find_element(By.ID, "status").text != "Done" and counter < 60:
            counter += 1
            time.sleep(1)
...
@click.option('-l', '--vpn_loc', default='Ann Arbor, MI, USA', help='Location of VPN (City, State, Country)')
@click.option('-y', '--your_loc', default='Ann Arbor, MI, USA', help='Your location (City, State, Country)')
def main(**kwargs):
    allowable_browsers = ["safari", "chrome", "firefox"]
